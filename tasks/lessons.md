...
- Icon progression for agents: ◇ (initial) → ◆ (queued) → ◈ (running) → ✓ (complete) / ✗ (error). Call line shows the icon, result line shows stats only (no icon). Both foreground and background agents use the same progression.
