...
describe("AgentManager", () => {
  ...
  function nextCompletion(): Promise<void> {
    return new Promise((resolve) => onComplete.mockImplementationOnce(() => resolve()));
  }

  async function settleRun(run: ReturnType<typeof makeResolvablePromise>, result: MockRunResult = mockRunResult()) {
  ...
  it("continues a settled agent by prompting its session, returning true immediately", async () => {
    ...
    await settleRun(deferred);
    expect(record.execution.settled).toBe(true);
  });
  ...
  it("skips re-reservation entirely when modelKey is undefined", async () => {
    ...
    await settleRun(deferred);
    expect(manager.getRecord(id)!.execution.settled).toBe(true);
  });
  ...
  it("releases the slot and drains the queue when the continuation settles", async () => {
    ...
    await settleRun(contRun);
    expect(manager.getRecord(idC)!.lifecycle.status).toBe("running");
    await settleRun(cRun);
    expect(manager.getRecord(idC)!.execution.settled).toBe(true);
  });
