 ...
 *
 * Ported from pi-subagents/src/agent-color.ts. Renders an agent's display
 * name as a colored badge: the configured color is the background, the text
 * an inverse foreground chosen by WCAG contrast. Named colors (Claude Code's
 * eight + Agency Agents palette) and 6-digit hex are supported. 256-color
 * terminals get quantized palette entries.
 */
...
/**
 * Wrap a pi-subagents-lite Theme into an AgentNameTheme for badge rendering.
 * The badge path uses raw ANSI sequences (bypassing Theme), so fg/bold are
 * only used in the no-color fallback path.
 */
export function themeForBadge(theme: Theme): AgentNameTheme {
  return {
