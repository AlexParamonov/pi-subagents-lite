...
async function initSession(
  ...
  // clamped to pi's minimum of 16; other APIs keep the pre-fix max_tokens
  // injection (it matches anthropic's native field; the other non-completions
  // fields are a pre-existing gap, issue #22).
  ...
  if (maxTokens != null && maxTokens > 0 && model) {
    ...
    session.agent.onPayload = async (payload, m) => {
      ...
      // instead of desyncing on a captured field.
