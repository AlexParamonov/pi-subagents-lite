...
- **Subagent output-limit injection follows pi's per-API algorithm.** When an agent sets `max_tokens`, the injected provider field is now resolved exactly like pi resolves it per API: openai-completions models follow the compat chain (an explicit `model.compat.maxTokensField` wins, otherwise the field is auto-detected from the provider name and baseUrl, defaulting to `max_completion_tokens`); OpenAI Responses models (`openai-responses`, `azure-openai-responses`) get pi's `max_output_tokens` field, clamped to pi's minimum of 16, with `openai-responses` skipped when the model disables `compat.supportsMaxOutputTokens` (matching newer pi-ai releases; the flag is inert on older ones); bedrock models nest the cap at `inferenceConfig.maxTokens`, both google APIs (`google-generative-ai`, `google-vertex`) nest it at `config.maxOutputTokens`, mistral uses top-level `maxTokens`, and pi-messages nests it at `options.maxTokens`. Codex models (`openai-codex-responses`) get no injection since pi sends no output-limit field there. Previously the extension defaulted to `max_tokens` whenever the model's compat was silent (openai-completions) or the model was not an openai-completions or Responses model at all, so those runs sent a field the provider ignores or rejects and the author's cap silently dropped. The field is resolved per request, so mid-run model changes stay in sync with pi; models on unknown future APIs keep the previous `max_tokens` injection.
