...
- **Agent `color` frontmatter for status icon tinting.** Agent `.md` files can set `color: <named-color>` or `color: #RRGGBB` in frontmatter. The agent's status icon (◆ queued, ◈ running, ✓ completed, ✗ error, ■ stopped) is tinted with that color across all UI surfaces: widget rows, tool call lines, subagent result messages, and the conversation viewer. Supported: 8 Claude Code named colors (red, blue, green, yellow, purple, orange, pink, cyan) and 15 Agency Agents palette aliases (amber, teal, indigo, gold, neon-green, neon-cyan, metallic-blue, violet, rose, lime, gray, grey, fuchsia, slate, navy). Invalid or missing color falls back to the default theme color.
